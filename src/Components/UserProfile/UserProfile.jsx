...
export default function UserProfile() {
  ...
  const getUserData = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`http://localhost:5555/app/user/getUser/${params.id}`, {
  ...
  const handleStartChat = async () => {
    try {
      const response = await axios.post(
        'http://localhost:5555/app/user/chating',
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:5555/app/user/sendMessagePrivte',
        {
          receiverID: params.id,
                ...
                <div className="mb-5 border p-4 shadow bg-light rounded text-center position-relative">
                  ...
                  <div className="row mb-4">
                    <div className="col-md-12">
                      <img
                        src={`http://localhost:5555${userData.image}`}
                        alt="Profile"
