...
const ProjectsCards = () => {
  ...
  async function fetchProjects() {
    setIsLoading(true);
    try {
      const allProjectsResponse = await axios.get('http://localhost:5555/app/project/getAllProjects');
  ...
  const handleUnlikeClick = async (event, project_id, like_id) => {
    ...
    try {
      ...
      if (token) {
        ...
        await axios.delete(
          `http://localhost:5555/app/projectInteractions/deleteMyLike/${project_id}`, // URL remains without like_id
          {
            withCredentials: true,
