...
export default function Navbar() {
  ...
  function Logout() {
    axios.get('http://localhost:5555/app/auth/logout')
      .then(() => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await axios.get('http://localhost:5555/app/Notification/showMyNotifications', {
  ...
  const clearAllNotifications = async () => {
    try {
      await axios.delete('http://localhost:5555/app/Notification/deleteAllMyNotification', {
        withCredentials: true,
      });
      setNotifications([]);
    } catch (error) {
