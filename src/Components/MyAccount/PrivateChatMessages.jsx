...
export default function PrivateChatMessages() {
  ...
  const fetchAllChats = async () => {
    try {
      const response = await axios.get('http://localhost:5555/app/user/showAllMyChats', { withCredentials: true });
  ...
  const fetchSpecificChatMessages = async (chatId) => {
    try {
      const response = await axios.get(`http://localhost:5555/app/user/showSpecificChat/${chatId}`, { withCredentials: true });
  ...
  const handleDeleteChat = async (chatId) => {
    try {
      await axios.delete('http://localhost:5555/app/user/deleteSpecificChat', {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5555/app/user/sendMessagePrivte', {
 ...
 <img
                        src={`http://localhost:5555${message.messageOwner.image}`}
                        alt="Profile"
