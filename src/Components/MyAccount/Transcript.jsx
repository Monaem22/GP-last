...
export default function Transcript() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:5555/app/user/showMyProfile', {
  ...
  const handleUpload = async (event) => {
    ...
    try {
      setUploading(true);
      await axios.post('http://localhost:5555/app/user/uploadTranscript', formData, {
        withCredentials: true,
        headers: {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete('http://localhost:5555/app/user/deleteTranscript', {
        withCredentials: true,
      });
    ...
    <div>
      ...
      <div style={styles.transcriptContainer}>
        {transcriptData ? (
          <>
            <iframe
              src={`http://localhost:5555${transcriptData}`}
              style={styles.iframe}
              title="Transcript"
            />
