...
const MyProjects = () => {
  ...
  async function fetchProjects() {
    ...
    try {
      ...
      if (token) {
        ...
        const myProjectsResponse = await axios.get('http://localhost:5555/app/project/AllmyProjects', {
  ...
  const fetchLikes = async (project_id, like_id) => {
    try {
      const response = await axios.get(`http://localhost:5555/app/projectInteractions/getAllLikes/${project_id}`);
  ...
  const handleImageClick = (event, pdfUrl) => {
    ...
    const pdfDownloadUrl = `http://localhost:5555/app/project/application-${applicationId}.pdf`;
    ...
    try {
      ...
      if (token) {
        if (likedProjects.includes(project_id)) {
          ...
          await axios.post(
            `http://localhost:5555/app/projectInteractions/addlike/${project_id}`,
            {
              isLiked: true,
            },
            {
  ...
  const handleUnlikeClick = async (event, project_id, like_id) => {
    ...
    try {
      ...
      if (token) {
        ...
        await axios.delete(
          `http://localhost:5555/app/projectInteractions/deleteMyLike/${project_id}/${like_id}`, // Include like_id in the URL
          {
            withCredentials: true,
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const handleDeleteClick = async (event, project_id) => {
    ...
    if (isConfirmed) {
      try {
        ...
        if (token) {
          await axios.delete(
            `http://localhost:5555/app/project/deleteMyProject/${project_id}`,
            {
              withCredentials: true,
              headers: { Authorization: `Bearer ${token}` },
            }
