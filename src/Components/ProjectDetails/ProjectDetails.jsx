...
export default function ProjectDetails() {
  ...
  async function getMyUserId() {
    try {
      const response = await axios.get('http://localhost:5555/app/user/showMyProfile', { withCredentials: true });
  ...
  async function getProjectDetails(id, userId) {
    try {
      const { data } = await axios.get(`http://localhost:5555/app/project/getSpacificProject/${id}`, {
      ...
      if (data.data.owner.image) {
        ...
        setOwnerImage(`http://localhost:5555${imageIdWithExtension}`);
      }
      
      const filename = data.data.pdf.split('/').pop(); // Extract filename from path
      setPdfUrl(`http://localhost:5555/app/project/${filename}`);
  ...
  const handleLike = async () => {
    try {
      ...
      if (token) {
        await axios.post(`http://localhost:5555/app/projectInteractions/addlike/${id}`, {}, {
          withCredentials: true,
    ...
    try {
      ...
      if (token) {
        if (wishlist.some(item => item._id === projectId)) {
          ...
          await axios.delete(
            `http://localhost:5555/app/userFavList/removeProjectFromFavoriteList`,
            {
          ...
          await axios.post(
            `http://localhost:5555/app/userFavList/addProjectToFavoriteList`,
  ...
  const handleDeleteProject = async () => {
    try {
      ...
      const token = getUserToken();
      if (token) {
        await axios.delete(`http://localhost:5555/app/project/deleteMyProject/${id}`, {
          withCredentials: true,
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDeleteProjectAdmin = async () => {
    try {
      ...
      const token = getUserToken();
      if (token) {
        await axios.delete(`http://localhost:5555/app/project/deleteProject/${id}`, {
          withCredentials: true,
          headers: { Authorization: `Bearer ${token}` },
        });
        toast.success('Project has been deleted succesfully');
    ...
    try {
      ...
      const token = getUserToken();
      if (token) {
        await axios.put(`http://localhost:5555/app/project/updateProjectData/${id}`, {
          projectName,
          category,
          description,
        }, {
  ...
  const handleCommentSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:5555/app/projectInteractions/addComment/${id}`, {
        content: comment,
      }, {
        withCredentials: true,
      });
  ...
  const fetchWishlist = async () => {
    try {
      ...
      const token = getUserToken();
      if (token) {
        const wishlistResponse = await axios.get('http://localhost:5555/app/userFavList/showMyFavoriteList', {
          withCredentials: true
        });
