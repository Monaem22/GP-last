...
export default function PendingProjects() {
  ...
  const fetchPendingProjects = async () => {
    try {
      const response = await axios.get('http://localhost:5555/app/project/AllPendingProjects', { withCredentials: true });
  ...
  const handleAcceptProject = async (projectId) => {
    try {
      ...
      await axios.post(`http://localhost:5555/app/project/changeProjectStatus/${projectId}`, { status: 'accepted' }, { withCredentials: true });
      fetchPendingProjects();
  ...
  const handleRejectProject = async (projectId) => {
    try {
      ...
      await axios.post(`http://localhost:5555/app/project/changeProjectStatus/${projectId}`, { status: 'rejected' }, { withCredentials: true });
      fetchPendingProjects();
          ...
          <tbody>
            {pendingProjects.map((project) => (
              <tr key={project._id}>
                ...
                <td>
                  <a href={`http://localhost:5555${project.pdf}`} target="_blank" rel="noopener noreferrer">
                    View PDF
                  </a>
                </td>
                <td>
