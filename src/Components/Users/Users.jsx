...
export default function Users() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:5555/app/user/getAllUser', { withCredentials: true });
  ...
  const fetchUserActions = async (userId) => {
    ...
    try { 
      const response = await axios.get(
        `http://localhost:5555/app/sysActions/showSysActionsOfSpecificUser/${userId}`,
  ...
  const deleteUser = async (userId, index) => {
    ...
    try {
      const response = await axios.delete(
        'http://localhost:5555/app/user/deleteUser',
        {
          data: { userId },
          withCredentials: true
        }
  ...
  const blockOrUnblockUser = async (userId, index, action) => {
    ...
    try {
      const response = await axios.post(
        action === 'block' ? 'http://localhost:5555/app/user/blockUser' : 'http://localhost:5555/app/user/unblockUser',
        { id: userId },
        { withCredentials: true }
      );
      if (response.status === 200 || response.status === 202) {
